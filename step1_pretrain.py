...
# ==============================================================================
# 1. REAL V16 GRAPH FEATURIZATION
# ==============================================================================
...
class FusionDataset(Dataset):
    def __init__(self, ds):
        self.smiles = ds.ids
        self.desc = torch.tensor(np.nan_to_num(ds.X.astype(np.float32)), dtype=torch.float32)
...
# ==============================================================================
# 2. V16 MASTER BACKBONE (The Core Brain)
# ==============================================================================
...
# ==============================================================================
# 3. TRAINING ROUTINE (Fixed for Offline mode)
# ==============================================================================
def train_pretraining_task(task_name, model, epochs, lr):
    print(f"\n{'='*50}\n>>> STAGE: Pre-training on {task_name.upper()}\n{'='*50}", flush=True)
    f_desc = dc.feat.RDKitDescriptors()
    
    # FIX: Pointing to local offline directories so it doesn't crash!
    try:
        if task_name == 'esol': 
            _, ds, _ = dc.molnet.load_delaney(featurizer=f_desc, data_dir='./esol_v4', reload=False)
        elif task_name == 'lipo': 
            _, ds, _ = dc.molnet.load_lipo(featurizer=f_desc, data_dir='./lipo_data_desc', reload=False)
    except Exception as e:
        # Fallback offline loader if deepchem molnet strictly needs internet
        print(f"Falling back to local CSV for {task_name}...", flush=True)
        if task_name == 'esol':
            loader = dc.data.CSVLoader(tasks=['measured log solubility in mols per litre'], feature_field='smiles', featurizer=f_desc)
            ds = [loader.create_dataset('./esol_data/delaney-processed.csv')]
        else:
            raise e

    norm_y = dc.trans.NormalizationTransformer(transform_y=True, dataset=ds[0])
    train_ds = norm_y.transform(ds[0])
    
    loader = DataLoader(FusionDataset(train_ds), batch_size=64, shuffle=True, collate_fn=custom_collate)
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        total_loss = 0
        for s, d, n, e, ew, b_idx, y in loader:
            s, d, n, e, ew, b_idx, y = s.to(device), d.to(device), n.to(device), e.to(device), ew.to(device), b_idx.to(device), y.to(device)
            opt.zero_grad()
        ...
        if epoch % 10 == 0:
            print(f"[{task_name.upper()}] Epoch {epoch:02d}/{epochs} | Average Loss: {total_loss/len(loader):.4f}", flush=True)

if __name__ == '__main__':
    # 1. Cluster offline hai, toh hum direct 217 features use karenge 
    # (Jo humein pichli error se pata chala tha!)
    actual_desc_dim = 217 
    print(f"\n>>> [OFFLINE MODE] Using Exact Descriptor Dimension: {actual_desc_dim}", flush=True)

    # 2. Model ko 217 dimension ke saath initialize karte hain
    backbone = NeuroChemBackbone(len(token_to_id)+2, desc_dim=actual_desc_dim) 
    wrapper = PretrainWrapper(backbone).to(device)

    # 3. Stage 1: Learn Basic Solubility (30 Epochs)
    train_pretraining_task('esol', wrapper, epochs=30, lr=5e-4)

    # 4. Stage 2: Learn Lipophilicity (Fat Mixing) (40 Epochs)
    train_pretraining_task('lipo', wrapper, epochs=40, lr=3e-4)

    # 5. Extract and Save ONLY the Brain (Backbone), discard the head
    print("\n>>> Pre-training Complete! Extracting Knowledge Backbone...", flush=True)
    torch.save(wrapper.backbone.state_dict(), 'master_chemistry_backbone.pt')
    print(">>> SUCCESS: 'master_chemistry_backbone.pt' saved! 🧠🔥", flush=True)
    print(">>> This brain is now ready to learn Biology (BBBP/Tox21) via Transfer Learning!", flush=True)