...
class AttentionGraphBranch(nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight, batch_idx, batch_size):
        ...
        # FIX: Per-graph attention softmax instead of global batch-wise softmax[cite: 2]
        attn_logits = self.attn(x)
        # Using a custom segmented softmax for per-graph normalization
        attn_scores = []
        for i in range(batch_size):
            mask = (batch_idx == i)
            if mask.any():
                attn_scores.append(torch.softmax(attn_logits[mask], dim=0))
...
print("\n>>> Starting V17 Tri-Modal Training on ESOL...", flush=True)
...
for epoch in range(1, 151): 
    ...
    for b_smiles, b_desc, b_nodes, b_edges, b_weights, b_batch_idx, b_y in train_loader:
        ...
        # FIX: Removed gradient centering loop as it interferes with AdamW internal averages[cite: 2]
    ...
    if epoch % 2 == 0:
        ...
        if v_r2 > best_r2:
            best_r2 = v_r2
            # UNIQUE Checkpoint name[cite: 2]
            torch.save(eval_model.state_dict(), 'esol_fusion_v17_best.pt')
            patience_counter = 0
        else:
            patience_counter += 2
...
state_dict = torch.load('esol_fusion_v17_best.pt')
...
print("\n" + "="*60)
print("ESOL FUSION V17 - FINAL TEST METRICS")
...
# ==============================================================================
# 5. Graph Generation (Enhanced Visualizations)
# ==============================================================================
print("\n>>> Generating Final Visualizations...")

# 1. Loss & R2 Curve
plt.figure(figsize=(10, 5))
plt.plot(history_epochs, history_train_loss, label='Train Loss', color='crimson')
plt.plot(history_epochs, history_val_r2, label='Val R2', color='royalblue')
plt.title('ESOL v17 Training Progress')
plt.xlabel('Epochs')
plt.ylabel('Metric Value')
plt.legend()
plt.grid(True, alpha=0.3)
plt.savefig('esol_v17_training_metrics.png', dpi=300)

# 2. Parity Plot
plt.figure(figsize=(7, 7))
sns.regplot(x=test_trues, y=test_preds, scatter_kws={'alpha':0.4, 'color':'teal'}, line_kws={'color':'red'})
plt.title(f'ESOL v17 Parity Plot (R2: {pearson_r2:.4f})')
plt.xlabel('Experimental Solubility')
plt.ylabel('Predicted Solubility')
plt.savefig('esol_v17_parity_plot.png', dpi=300)

# 3. Modality Contribution (Bar Plot)
plt.figure(figsize=(8, 5))
labels = ['Sequence (Mamba)', 'Physics (RDKit)', 'Graph (GNN)']
colors = ['#3498db', '#9b59b6', '#2ecc71']
plt.bar(labels, avg_gates, color=colors, edgecolor='black', alpha=0.8)
plt.title('Modality Importance Gating (ESOL)')
plt.ylabel('Average Gate Weight')
plt.ylim(0, 1)
plt.savefig('esol_v17_gate_weights.png', dpi=300)

print(">>> Visualization saved: esol_v17_training_metrics.png, esol_v17_parity_plot.png, esol_v17_gate_weights.png")