...
# ==============================================================================
# 1. Custom Graph Featurization & Data Loading
# ==============================================================================
...
# ==============================================================================
# 2. Architectures (Fixed GNN & Modality Masking)
# ==============================================================================
...
class ResGraphBranch(nn.Module):
    """ADDED: LayerNorm and Residuals so the GNN gradients don't die!"""
    ...
    def _pass_messages(self, x, edge_index, layer, norm):
        if edge_index.shape[1] > 0:
            ...
            # Residual + LayerNorm = Alive Gradients
...
class NeuroChemFusionV12(nn.Module):
    ...
    def forward(self, smiles_ids, descriptors, nodes, edges, batch_idx, return_gates=False):
        ...
        # Modality Masking during Training (Prevents Mamba Memorization)
        if self.training:
...
model = NeuroChemFusionV12(vocab_size=vocab_size, desc_dim=train_ds.X.shape[1]).to(device)

# Differential LR: Slow down Mamba so GNN can catch up!
optimizer = torch.optim.AdamW([
    {'params': model.mamba_branch.parameters(), 'lr': 1e-4}, 
    {'params': model.embedding.parameters(), 'lr': 1e-4},
    {'params': model.graph_branch.parameters(), 'lr': 5e-4}, # Boost GNN LR
...
print("\n>>> Starting V12 Training (With Early Stopping & ResGNN)...", flush=True)
...
for epoch in range(1, 101):
    ...
    if epoch % 2 == 0:
        ...
        if v_r2 > best_r2:
            best_r2 = v_r2
            torch.save(model.state_dict(), 'neurochem_v12_best.pt')
            patience_counter = 0
        else:
            patience_counter += 2
...
model.load_state_dict(torch.load('neurochem_v12_best.pt'))
model.eval()
...
print("\n" + "="*60)
print("NEUROCHEM FUSION V12 (RES-GNN + EARLY STOPPING) - FINAL RESULTS")
