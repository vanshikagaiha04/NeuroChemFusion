...
# ==============================================================================
# 1. IMPROVED Data Preprocessing & Graph Featurization
# ==============================================================================
def smiles_to_graph(smiles):
    ...
    for atom in mol.GetAtoms():
        ...
        atomic_num = atom.GetAtomicNum() / 100.0  # Max is ~118, keeps it <= 1.2
        degree = atom.GetDegree() / 6.0           # Max bonds usually 4-6
        num_hs = atom.GetTotalNumHs() / 4.0       # Max usually 3-4
        is_aromatic = float(atom.GetIsAromatic())
        
        features = [atomic_num, degree, num_hs, is_aromatic]
        features += [0.0] * (10 - len(features))  # Pad to 10 dims
    ...
    for bond in mol.GetBonds():
        ...
        edges += [[i, j], [j, i]]
    edge_index = np.zeros((2, 0), dtype=np.int64) if len(edges) == 0 else np.array(edges, dtype=np.int64).T
...
def prepare_data():
    ...
    # ScaffoldSplitter is harder than RandomSplit, which is good for real-world testing!
...
class FusionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        node_feat, edge_idx = smiles_to_graph(self.smiles[idx])
...
def custom_collate(batch):
    ...
    batch_nodes, batch_edges, batch_indices, node_offset = [], [], [], 0
    for i, (nodes, edges) in enumerate(zip(b_nodes, b_edges)):
        batch_nodes.append(torch.tensor(nodes, dtype=torch.float32))
        if edges.shape[1] > 0: batch_edges.append(torch.tensor(edges, dtype=torch.long) + node_offset)
...
class ResGraphBranch(nn.Module):
    def __init__(self, node_dim=10, hidden_dim=256): # WIDENED TO 256
        super().__init__()
    ...
    def _pass_messages(self, x, edge_index, layer, norm):
    ...
    def forward(self, x, edge_index, batch_idx, batch_size):
        x = self.node_proj(x)
        x = self._pass_messages(x, edge_index, self.mp1, self.norm1)
        x = self._pass_messages(x, edge_index, self.mp2, self.norm2)
        x = self._pass_messages(x, edge_index, self.mp3, self.norm3)

        graph_embeds = torch.zeros(batch_size, x.shape[1], device=x.device)
        counts, ones = torch.zeros(batch_size, 1, device=x.device), torch.ones(batch_idx.size(0), 1, device=x.device)
        graph_embeds.index_add_(0, batch_idx, x)
        counts.index_add_(0, batch_idx, ones)
        return self.out_proj(graph_embeds / counts.clamp(min=1))
...
class NeuroChemFusionV13(nn.Module):
    def __init__(self, vocab_size, desc_dim):
        ...
        self.graph_branch = ResGraphBranch()
...
# ==============================================================================
# 3. Training Loop with Better Patience
# ==============================================================================
...
model = NeuroChemFusionV13(vocab_size=vocab_size, desc_dim=train_ds.X.shape[1]).to(device)
...
# INCREASED PATIENCE to 10 so it fine-tunes longer
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.5, patience=10)

print("\n>>> Starting V13 Training (Scaled Nodes + Wider GNN)...", flush=True)

best_r2 = -float('inf')
patience_counter = 0
EARLY_STOP_PATIENCE = 20 # Wait longer before killing the run

for epoch in range(1, 121): # Bumped max epochs slightly
    model.train()
    epoch_loss = 0.0
    ...
    if epoch % 2 == 0:
        ...
        if v_r2 > best_r2:
            best_r2 = v_r2
            torch.save(model.state_dict(), 'neurochem_v13_best.pt')
            patience_counter = 0
        else:
            patience_counter += 2
...
model.load_state_dict(torch.load('neurochem_v13_best.pt'))
model.eval()
...
print("\n" + "="*60)
print("NEUROCHEM FUSION V13 (PREPROCESSED + WIDE GNN) - FINAL RESULTS")
