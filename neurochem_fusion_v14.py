...
# ==============================================================================
# 1. ADDED: Bond Weights (Edge Features) & Normalization
# ==============================================================================
def smiles_to_graph(smiles):
    ...
    for atom in mol.GetAtoms():
        ...
        features = [atomic_num, degree, num_hs, is_aromatic] + [0.0]*6
    ...
    for bond in mol.GetBonds():
        ...
        edges += [[i, j], [j, i]]
        # Bond weight: Single=1.0, Double=2.0, Triple=3.0, Aromatic=1.5
...
# ==============================================================================
# 2. Architectures (Weighted Edges + Attention Pooling)
# ==============================================================================
...
class AttentionGraphBranch(nn.Module):
    ...
    def _pass_messages(self, x, edge_index, edge_weight, layer, norm):
        if edge_index.shape[1] > 0:
            row, col = edge_index
            # Multiply message by the chemical bond weight (Single, Double, Aromatic)
    ...
    def forward(self, x, edge_index, edge_weight, batch_idx, batch_size):
        ...
        # Global Attention Pooling (Focus on important atoms)
        attn_scores = torch.softmax(self.attn(x), dim=0) # Simple approximation
...
class NeuroChemFusionV14(nn.Module):
    ...
    def forward(self, smiles_ids, descriptors, nodes, edges, edge_weights, batch_idx, return_gates=False):
...
# ==============================================================================
# 3. Training Loop with SWA (Stochastic Weight Averaging)
# ==============================================================================
...
model = NeuroChemFusionV14(vocab_size=vocab_size, desc_dim=train_ds.X.shape[1]).to(device)
...
# ADDED: SWA to smooth out the Val-Test generalization gap
swa_model = AveragedModel(model)
swa_scheduler = SWALR(optimizer, swa_lr=1e-4)
SWA_START_EPOCH = 50 

print("\n>>> Starting V14 Training (Bond Weights + Attention GNN + SWA)...", flush=True)
...
for epoch in range(1, 121):
    ...
    for b_smiles, b_desc, b_nodes, b_edges, b_weights, b_batch_idx, b_y in train_loader:
        ...
        optimizer.zero_grad()
        preds = model(b_smiles, b_desc, b_nodes, b_edges, b_weights, b_batch_idx)
    ...
    if epoch % 2 == 0:
        ...
        if v_r2 > best_r2:
            best_r2 = v_r2
            torch.save(eval_model.state_dict(), 'neurochem_v14_best.pt')
            patience_counter = 0
        else:
            patience_counter += 2
            
        if patience_counter >= 30: # Extra patience for SWA
            print(f"\n[!] Early Stopping triggered at Epoch {epoch} (Peak R² was {best_r2:.4f})")
            break

# Update BatchNorm stats for SWA
...
final_eval_model = AveragedModel(model) if os.path.exists('neurochem_v14_best.pt') else model
final_eval_model.load_state_dict(torch.load('neurochem_v14_best.pt'))
...
with torch.no_grad():
    for b_smiles, b_desc, b_nodes, b_edges, b_weights, b_batch_idx, b_y in test_loader:
        b_smiles, b_desc = b_smiles.to(device), b_desc.to(device)
        b_nodes, b_edges, b_weights, b_batch_idx = b_nodes.to(device), b_edges.to(device), b_weights.to(device), b_batch_idx.to(device)
        # AveragedModel wraps the internal model as `module`
...
print("\n" + "="*60)
print("NEUROCHEM FUSION V14 (BOND WEIGHTS + SWA) - FINAL RESULTS")
