...
# ==============================================================================
# 1. FEATURIZATION & DATA LOADING (LIPO)
# ==============================================================================
...
def prepare_data():
    ...
    print(">>> Loading Lipophilicity Dataset for Evaluation...", flush=True)
    # Use the same exact loading method as your original V16 script
    ...
    # Lipo used ScaffoldSplitter (Hard Mode)
...
print(">>> Loading neurochem_v16_best.pt...", flush=True)
# Load the original Lipo model weights
state_dict = torch.load('neurochem_v16_best.pt', map_location=device)
...
print("\n" + "="*60)
print("ADVANCED CLASSIFICATION METRICS (BINARIZED LIPO)")
print("="*60)
...
# 4. PLOTTING
# ==============================================================================
print(">>> Generating Advanced Classification Plots for Lipo...", flush=True)

# PLOT 1: Score Distribution
plt.figure(figsize=(7, 5))
plt.hist(test_trues, bins=30, alpha=0.6, color='blue', label='Actual Lipophilicity', density=True)
plt.hist(test_preds, bins=30, alpha=0.6, color='green', label='Predicted Lipophilicity', density=True)
plt.axvline(x=threshold, color='red', linestyle='dashed', linewidth=2, label=f'Median Split Threshold')
plt.title('Prediction Score Distribution Density (Lipo Task)')
plt.xlabel('Normalized Lipophilicity')
plt.ylabel('Density')
plt.legend()
plt.tight_layout()
plt.savefig('lipo_plot_1_score_distribution.png', dpi=300)
plt.close()
...
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate (Predicted High Lipo, Actually Low)')
plt.ylabel('True Positive Rate (Correctly predicted High Lipo)')
plt.title('Receiver Operating Characteristic (ROC) Curve - Lipo')
plt.legend(loc="lower right")
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('lipo_plot_2_roc_curve.png', dpi=300)
plt.close()

# PLOT 3: Precision-Recall Curve
plt.figure(figsize=(6, 6))
plt.plot(recall, precision, color='purple', lw=2, label=f'PR curve (AUC = {pr_auc:.3f})')
plt.xlabel('Recall (Fraction of High Lipo molecules found)')
plt.ylabel('Precision (Confidence when predicting High Lipo)')
plt.title('Precision-Recall Curve - Lipo')
plt.legend(loc="lower left")
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('lipo_plot_3_pr_curve.png', dpi=300)
plt.close()

# PLOT 4: Confusion Matrix
disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=['Low Lipo', 'High Lipo'])
fig, ax = plt.subplots(figsize=(6, 6))
disp.plot(cmap='Greens', ax=ax, values_format='d')
plt.title('Confusion Matrix (Binarized Lipo Predictions)')
plt.tight_layout()
plt.savefig('lipo_plot_4_confusion_matrix.png', dpi=300)
plt.close()

print(">>> SUCCESS! 4 New Graphs generated successfully.")
print(">>> Saved as: lipo_plot_1_score_distribution.png, lipo_plot_2_roc_curve.png, lipo_plot_3_pr_curve.png, lipo_plot_4_confusion_matrix.png")