...
# Added for plotting on headless cluster
import matplotlib
matplotlib.use('Agg') 
...
print("\n>>> Starting V16 Training (With Metric Tracking & Graph Generation)...", flush=True)
...
for epoch in range(1, 121): 
    ...
    if epoch % 2 == 0:
        ...
        if v_r2 > best_r2:
            best_r2 = v_r2
            torch.save(eval_model.state_dict(), 'neurochem_v16_best.pt')
            patience_counter = 0
        else:
            patience_counter += 2
...
state_dict = torch.load('neurochem_v16_best.pt')
...
# Calculate Complete Suite of Metrics
...
print("\n" + "="*60)
print("NEUROCHEM FUSION V16 - FINAL TEST METRICS")
...
# Graph 1: Training Curve (Loss vs Validation R2)
...
plt.title('NeuroChem Training Curve (Loss & Validation R²)')
fig.tight_layout()
plt.savefig('neurochem_training_curve.png', dpi=300)
plt.close()

# Graph 2: Actual vs Predicted Parity Plot
plt.figure(figsize=(6, 6))
plt.scatter(test_trues, test_preds, alpha=0.5, color='purple', edgecolors='k')
...
plt.xlabel('Actual Experimental Lipophilicity (Normalized)')
plt.ylabel('Predicted Lipophilicity')
plt.title(f'Parity Plot (Test R²: {pearson_r2:.4f})')
plt.legend()
plt.grid(True, linestyle='--', alpha=0.6)
plt.tight_layout()
plt.savefig('neurochem_parity_plot.png', dpi=300)
plt.close()

# Graph 3: Modality Gate Weights Bar Chart
...
plt.title('Tri-Modal Asymmetric Fusion Contributions')
...
plt.tight_layout()
plt.savefig('neurochem_gate_weights.png', dpi=300)
plt.close()

print(">>> Graphs saved successfully: neurochem_training_curve.png, neurochem_parity_plot.png, neurochem_gate_weights.png", flush=True)